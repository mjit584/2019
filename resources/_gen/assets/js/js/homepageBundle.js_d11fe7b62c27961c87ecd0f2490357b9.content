...
function prepareDom(){css(container,{'height':'100%','position':'relative'});addClass(container,WRAPPER);addClass($('html'),ENABLED);windowsHeight=getWindowHeight();removeClass(container,DESTROYED);addInternalSelectors();var sections=$(SECTION_SEL);for(var i=0;i<sections.length;i++){var sectionIndex=i;var section=sections[i];var slides=$(SLIDE_SEL,section);var numSlides=slides.length;section.setAttribute('data-fp-styles',section.getAttribute('style'));styleSection(section,sectionIndex);styleMenu(section,sectionIndex);}
...
slides.forEach(function(slide){css(slide,{'width':slideWidth+'%'});});var startingSlide=$(SLIDE_ACTIVE_SEL,section)[0];if(startingSlide!=null&&(index($(SECTION_ACTIVE_SEL),SECTION_SEL)!==0||(index($(SECTION_ACTIVE_SEL),SECTION_SEL)===0&&index(startingSlide)!==0))){silentLandscapeScroll(startingSlide,'internal');}else{addClass(slides[0],ACTIVE);}}
...
console.log("cards");$('.card').hover(function(){$(this).toggleClass('hovered');},function(){$(this).toggleClass('hovered');})
$('.cards').hover(function(){$(this).toggleClass('darkened')},function(){$(this).toggleClass('darkened');})
if($(window).width()<800){$('#fullpage').removeAttr('id');};new fullpage('#fullpage',{autoScrolling:true,anchors:['firstPage','secondPage','thirdPage']});console.log('homepage js linked');console.log('testing to make sure headers.js is linked up');var prevScrollpos=window.pageYOffset;window.onscroll=function(){var currentScrollPos=window.pageYOffset;if(prevScrollpos>currentScrollPos){document.getElementById("header").style.top="0";}else{document.getElementById("header").style.top="-75px";}
prevScrollpos=currentScrollPos;}
var storyDropdown=document.getElementById('story-dropdown');var mainMenu=document.getElementById('main-menu');storyDropdown.onmouseenter=function(){storyDropdown.classList.toggle('is-active');mainMenu.classList.toggle('open');}
